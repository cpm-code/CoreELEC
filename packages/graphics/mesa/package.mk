...
post_makeinstall_target() {
  # Amlogic-ng uses the proprietary libMali EGL/GLES stack from opengl-meson.
  # Keep Mesa's GBM runtime, but do not let Mesa overwrite the runtime EGL/GLES
  # sonames in the final image.
  if [ "${OPENGLES}" = "opengl-meson" ]; then
    safe_remove ${INSTALL}/usr/lib/libEGL.so*
    safe_remove ${INSTALL}/usr/lib/libGLESv2.so*
  fi
}
