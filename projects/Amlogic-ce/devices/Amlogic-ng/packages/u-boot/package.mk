...
makeinstall_target() {
  ...
  # Copy Hardkernel boot logo
	if find_file_path splash/${DEVICE}/hk-boot-logo-1080.bmp.gz; then
	  cp -av ${FOUND_PATH} $INSTALL/usr/share/bootloader
	fi
	# Copy Radxa boot logo
	if find_file_path splash/${DEVICE}/radxa-boot-logo-1080.bmp.gz; then
	  cp -av ${FOUND_PATH} $INSTALL/usr/share/bootloader
	fi
